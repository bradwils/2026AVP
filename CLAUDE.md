...
- Compartmentalize work into separate files as much as possible — e.g. split a feature's observable model and its SwiftUI view into their own files (see `MinuteTimerModel.swift` / `MinuteTimerView.swift`) rather than growing existing files like `ContentView.swift` or `AppModel.swift`. This project uses Xcode's file-system-synchronized groups, so new files dropped into `AVPGroupApp/AVPGroupApp/` are automatically added to the build target — no project file edits needed.
